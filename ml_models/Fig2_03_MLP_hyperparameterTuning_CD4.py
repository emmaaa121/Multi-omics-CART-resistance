...
def objective(trial):
    ...
    for i, layer_size in enumerate(hidden_layers):
        if i == 0:
            dropout_rates.append(trial.suggest_float(f'dropout_first', 0.2, 0.4))
        elif i == len(hidden_layers) - 1:
            dropout_rates.append(trial.suggest_float(f'dropout_last', 0.1, 0.3))
        else:
            dropout_rates.append(trial.suggest_float(f'dropout_{i}', 0.2, 0.5))
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_val, y_train_val)):
        ...
        model = NeuralNet(
            input_size=X_train_val_scaled.shape[1],
        ...
        for epoch in range(num_epochs):
            ...
            model.eval()
            with torch.no_grad():
                val_pred = model(X_val)  # Shape: [val_size, 1]
                val_loss = criterion(val_pred.squeeze(), y_val.view(-1))  # Ensure both are 1D
...
optimizer = optim.AdamW(final_model.parameters(), lr=best_params['lr'], weight_decay=0.01)
criterion = nn.BCELoss(weight=None)
...
for epoch in range(best_params['num_epochs']):
    ...
    for i in range(0, len(X_train_val_tensor), best_params['batch_size']):
        ...
        outputs = final_model(batch_X)  # Shape: [batch_size, 1]
        weights = class_weights[batch_y.long().squeeze()]
        loss = criterion(outputs.squeeze(), batch_y.view(-1)) * weights.mean()  # Ensure both are 1D
